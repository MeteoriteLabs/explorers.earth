...
checksummed `0001_runtime_baseline` and `0002_identity_lifecycle` migration
chain, creating all 27 runtime tables on PostgreSQL 15. The migration-owned
`music_schema_migrations` journal and `music_identity_tombstones` durable
identity-denial table are control tables outside the runtime-reference count.
Catalog fingerprints include tables, columns, constraints, indexes, triggers,
and sequences. `drizzle-kit push` remains forbidden.
...
No retention duration is invented; durable retention-stage fields are policy
inputs for C8.
