...
`0006_numeric_identity_lock` migration
...
identity-denial table, and `music_identity_lifecycle_operations` idempotency
history are control tables outside the runtime-reference count. Catalog
fingerprints include tables, columns, constraints, indexes, trigger definitions,
...
immutable Strapi user/selected Account document IDs, mutable display snapshots,
