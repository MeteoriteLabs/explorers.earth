...
`0015_publication_operation_archive` migration (following durable reactivation
authority, the immutable `0011_durable_publication_idempotency` authority, and
the append-only `0012_publication_replay_expiry_guard` correction)
...
the larger live row. Catalog fingerprints include
