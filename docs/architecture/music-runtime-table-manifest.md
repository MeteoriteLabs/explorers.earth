...
`0013_publication_operation_database_clock` migration (following the immutable
`0011_durable_publication_idempotency` authority and the append-only `0012_publication_replay_expiry_guard` correction)
...
operation history are control tables outside the runtime-reference count. The publication
control table retains hashed operation tombstones and encrypted, 24-hour responses; it never
