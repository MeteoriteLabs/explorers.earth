...
# UI/UX Implementation Guide - Apps & Tools Feature

This document serves as the "Source of Truth" for the visual and interactive implementation of the Apps & Tools feature. Following these standards ensures 90%+ accuracy and reduces the need for manual UI fixes.
...
### Hero Sections (Top Apps / Top Picks)
- **Clickability**: The entire Hero card MUST be clickable to open details (`onAppClick`).
- **Management Mode**: When in the dashboard/management view, the "See Details" button should be replaced with a **"Manage Top Apps"** button using `bg-dashboard-accent`.
- **Layout**: Square/Rounded-Rect aspect ratio (1:1) for App/Tool logos, and 16:9 aspect ratio for screenshots. If in management mode, ensure a prominent button is visible for the "Manage" action.
...
- Logos: Render app logos inside a rounded wrapper with `overflow-hidden border border-white/10 shadow-sm`.
