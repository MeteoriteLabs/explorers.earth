...
# UI/UX Implementation Guide - Movies & Shows Feature

This document serves as the "Source of Truth" for the visual and interactive implementation of the Movies & Shows feature. Following these standards ensures 90%+ accuracy and reduces the need for manual UI fixes.
...
- **Clickability**: The entire Hero card MUST be clickable to open details.
...
- **Layout**: Landscape/Cinematic aspect ratio for movies. If in management mode, ensure a prominent button is visible for the "Manage" action.

### Cards (Movie List Cards)
...
1. **Yellow Overrides**: Avoid `bg-amber-x` or `bg-blue-600/70` for primary actions. Always use the standard `dashboard-accent`.
