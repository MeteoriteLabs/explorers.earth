...
| TC-024 | Submit with note and rating saves movie to Strapi | User has movie selected, note entered, rating selected | 1. Enter note text (rich text format)<br/>2. Select 4 stars for user rating<br/>3. Click "Add Movie"<br/>4. Verify in database | Movie saved to Strapi with tmdb_id, title, year, director, genres, rating, note, user_rating: 4, where_to_watch, and list association | P0 |
...
| TC-029 | Media upload works (direct to S3) | User on Add Movie form with movie selected | 1. Click snapshot upload area<br/>2. Select image from device<br/>3. Upload completes<br/>4. Verify in detail view | Image uploaded successfully to S3 folder; preview shows in Add Movie form; persists when movie is saved | P1 |
...
| ID | Description | Preconditions | Steps | Expected Result | Priority |
|---|---|---|---|---|---|
| TC-040 | Edit movie note and rating | User has list with a movie | 1. Open list<br/>2. Click movie card/edit icon<br/>3. Change note text<br/>4. Change user rating stars<br/>5. Save changes | Note and User Rating updated in Strapi; change persists when page reloads | P0 |
...
| TC-121 | Modal shows all metadata | Detail modal open with movie | 1. Observe all fields in modal<br/>2. Verify completeness | Modal displays: poster image, title, release year, TMDB rating, user rating (stars), genre tags, director, runtime | P0 |
| TC-122 | Creator's note and rating displayed | Movie has creator note and rating | 1. Open detail modal<br/>2. Observe highlighted section | Note displayed with Tiptap formatting; user rating stars displayed prominently | P0 |
