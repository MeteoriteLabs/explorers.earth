...
async function runTemplateMutation(
  ...
  body.variables.data.social_media = socialMedia;
...
async function normalExactRestore(
  ...
  baselineSocialMedia: Record<string, unknown>,
) {
  ...
  const handler = async (route: Route) => {
    ...
    body.variables.data.social_media = baselineSocialMedia;
...
test.describe('approved live profile writes', () => {
  ...
  test('publishes the pairwise matrix and restores exact raw social_media', async ({
    ...
    const sentinelSocialMedia = clone(baselineSocialMedia) as Record<string, any>;
    ...
    try {
      ...
      await runTemplateMutation(page, template, sentinelSocialMedia);
      ...
      if (liveWriteStarted) {
        await restoreWithEmergency({
          ...
          emergencyRestore: async () =>
            runTemplateMutation(page, template, baselineSocialMedia),
          verify: async () => {
