...
type ErrorKind = "search" | "lookup" | "queue" | null;
type PendingPlay = { songId: number; youtubeId: string; title: string };
...
export function MusicSearch({ searchClient, queueClient, onChanged }: MusicSearchProps) {
  ...
  const invalidateDiscovery = () => { generation.current += 1; setSearchBusy(false); setResults(null); setNextPageToken(null); setSelected(new Set()); setError(null); };
  const chooseMode = (next: Mode) => { if (next !== mode) invalidateDiscovery(); setMode(next); };
  const onTabKey = (event: KeyboardEvent<HTMLButtonElement>) => {
    let next: Mode | null = null;
    if (event.key === "ArrowRight" || event.key === "End") next = "url";
    if (event.key === "ArrowLeft" || event.key === "Home") next = "search";
    if (!next) return;
  ...
  async function runSearch(pageToken?: string) {
    ...
    const requestGeneration = ++generation.current; setSearchBusy(true); setError(null);
    try {
    ...
    finally { if (generation.current === requestGeneration) setSearchBusy(false); }
  }
  ...
  async function retryPlaying(play: PendingPlay) {
    ...
    try { await queueClient.setPlaying(play.songId, key("play")); setPendingPlay(null); await onChanged(); }
  ...
  async function add(videos: YouTubeVideo[], playNow = false) {
    ...
    try {
      for (const video of videos) {
        ...
        if (playNow) {
          const play = { songId: added.id, youtubeId: video.id.videoId, title: video.snippet.title };
          setPendingPlay(play);
          try { await queueClient.setPlaying(added.id, key("play")); setPendingPlay(null); }
  ...
  const errorText = error === "search" ? "Music search is temporarily unavailable. Try again." : error === "lookup" ? "Music lookup is temporarily unavailable. Try again." : error === "queue" ? "Queue update failed. The current queue was reloaded." : null;
  return <section aria-labelledby="music-search-heading" className="space-y-4">
    ...
    <div role="tabpanel" id={mode === "search" ? "music-search-panel" : "music-url-panel"} aria-labelledby={mode === "search" ? "music-search-tab" : "music-url-tab"}>
      {mode === "search" ? <form onSubmit={(event) => { event.preventDefault(); void runSearch(); }} className="flex gap-2">
        ...
        <button type="submit" disabled={!query.trim()} className="min-h-11 px-4">Search</button>
        ...
        <button type="submit" disabled={!url.trim()} className="min-h-11 px-4">Look up</button>
      </form>}
    </div>
    {errorText && <div role="alert" className="space-y-2"><p>{errorText}</p>{error === "search" && <button type="button" onClick={() => void runSearch()} className="min-h-11 px-4">Try search again</button>}</div>}
    ...
    {results && results.length > 0 && <>
      <ul className="space-y-2">{results.map((video) => { const retry = pendingPlay?.youtubeId === video.id.videoId; return <li key={video.id.videoId} className="flex items-center gap-3">
        ...
        <button type="button" disabled={mutationBusy} onClick={() => retry ? void retryPlaying(pendingPlay) : void add([video], true)} aria-label={retry ? `Retry playing ${video.snippet.title}` : `Play ${video.snippet.title} now`} className="min-h-11 min-w-11 px-3">{retry ? "Retry play" : "Play now"}</button>
      </li>; })}</ul>
