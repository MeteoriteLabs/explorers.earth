...
export interface MusicWorkspaceShellProps {
  ...
  loading?: boolean; stale?: boolean; empty?: boolean; onAddFirstSong?: () => void;
}

export function MusicWorkspaceShell({ player, search, queue, history, loading = false, stale = false, empty = false, onAddFirstSong }: MusicWorkspaceShellProps) {
  ...
  const onKey = (event: KeyboardEvent<HTMLButtonElement>, index: number) => {
    ...
    const next = event.key === "Home" || (event.key === "ArrowLeft" && index === 1) ? "queue" : "history";
    choose(next, true);
  };
  return <div role="region" aria-label="Music workspace" aria-readonly={stale || undefined} className="relative max-w-full overflow-x-hidden pb-[calc(5rem+env(safe-area-inset-bottom))] md:pb-0">
    ...
    <section aria-label="Music player region" className={`${mobile === "player" ? "block" : "hidden"} sticky top-0 z-20 rounded-2xl border border-dashboard bg-dashboard-sidebar p-4 shadow-lg md:block md:p-5`}>{player}</section>
    <section className={`${mobile === "search" ? "block" : "hidden"} mt-5 rounded-2xl border border-dashboard bg-dashboard-sidebar p-4 md:block md:p-5`}>{search}</section>
    {empty && <section className="mt-5 rounded-2xl border border-dashboard bg-dashboard-sidebar p-6 text-center"><h2 className="text-xl font-semibold">Your queue is ready</h2><p className="mt-2 text-dashboard-light">Find something you love and start listening.</p><button type="button" onClick={onAddFirstSong} className="mt-4 min-h-11 min-w-11 rounded-xl bg-dashboard-accent px-4 font-semibold text-[var(--dash-accent-text)]">Add your first song</button></section>}
    ...
    <nav aria-label="Music workspace" className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-4 border-t border-dashboard bg-dashboard-sidebar pb-[env(safe-area-inset-bottom)] md:hidden">
      {(["player", "queue", "search", "more"] as const).map((value) => <button key={value} type="button" aria-current={mobile === value ? "page" : undefined} onClick={() => setMobile(value)} className="min-h-11 min-w-11 px-2 py-3 text-sm capitalize focus-visible:ring-2">{value[0].toUpperCase() + value.slice(1)}</button>)}
    </nav>
  </div>;
}
