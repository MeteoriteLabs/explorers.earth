...
const LISTS = [
  {
    ...
    status: 'Published',
    items: [
      { name: 'Interstellar', meta: 'Nolan • 2014', img: '/landing/Bali.jpg' },
      { name: 'Inception', meta: 'Nolan • 2010', img: '/landing/Paris.jpg' },
      { name: 'Tenet', meta: 'Nolan • 2020', img: '/landing/Kyoto.jpg' },
      { name: 'The Prestige', meta: 'Nolan • 2006', img: '/landing/Eiffel_Tower.jpg' },
    ],
  },
  {
    name: 'Classics',
    ...
    status: 'Draft',
    items: [
      { name: 'The Godfather', meta: 'Coppola • 1972', img: '/landing/Louvre_Museum.jpg' },
      { name: 'Schindler\'s List', meta: 'Spielberg • 1993', img: '/landing/Bali.jpg' },
    ],
  },
  {
    name: 'Watch Later',
                ...
                <div key={list.name} className="bg-[#0B0B0E] border border-white/5 rounded-xl p-2.5">
                  ...
                  {list.items.length > 0 ? (
                    ...
                    <div className="h-8 rounded-md bg-white/5 border border-dashed border-white/10 flex items-center justify-center mb-2">
                      <span className="text-[7px] text-gray-500">No movies yet</span>
                    </div>
                  )}
                  <div className="flex items-center justify-between">
                    <span className="text-[8px] text-gray-500">{list.count} movies</span>
        ...
        {(phase === 1 || phase === 2) && (
          <motion.div key="detail" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.3 }}
            ...
            <div className="px-3 py-2.5 flex items-center gap-2 shrink-0 border-b border-white/5">
              ...
              <Film size={12} className="text-[#3498DB] shrink-0" />
              <div className="flex-1 min-w-0">
                <h2 className="text-[10px] font-black text-white truncate">{activeList.name}</h2>
                <p className="text-[7px] text-white/40 uppercase tracking-widest font-bold">{activeList.count} movies • Movies</p>
              </div>
            </div>
              ...
              {phase === 2 && (
                <motion.div key="manage" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  ...
                  <div>
                    ...
                    <p className="text-[7px] text-white/40 uppercase tracking-[0.2em] font-bold mt-0.5">{activeList.name} • {activeList.count} movies</p>
                  </div>
