...
import { Gamepad2, Plus, Star, ChevronRight, X, Loader2, Search } from 'lucide-react';
import { AddIcon } from '../../../../../assets/icons/AddIcon';

// Stages:
// 0: View home with list cards (2s)
// 1: Hover card 1 (1.2s)
// 2: Click "New List" button press (0.5s)
// 3: Modal open + type name (2s)
// 4: Loading submit (0.8s)
// 5: New card appeared + list view opens (0.5s)
// 6: Inside list: search bar with typing + result appears (2.5s)
// 7: Click "Add" on a game result (1s)
// 8: Game added to list, visible as cover art (2s)
// → loop

const STAGES = [2000, 1200, 500, 2000, 800, 500, 2500, 1000, 2000];

export default function GamesMockup() {
  ...
  useEffect(() => {
    ...
    t = setTimeout(() => {
      if (next === 0) { setTypedListName(''); setTypedSearch(''); setGameAdded(false); }
      setStage(next);
    }, STAGES[stage]);
    return () => clearTimeout(t);
  }, [stage]);

  // Typing list name in stage 3
  useEffect(() => {
    if (stage !== 3) return;
    const target = 'My Favourite RPGs';
    let i = typedListName.length;
    const iv = setInterval(() => {
      i++;
      setTypedListName(target.slice(0, i));
      if (i >= target.length) clearInterval(iv);
    }, 70);
    return () => clearInterval(iv);
  }, [stage]);

  // Typing search in stage 6
  useEffect(() => {
    if (stage !== 6) return;
    const target = 'Elden Ring';
    let i = 0;
    const iv = setInterval(() => {
      i++;
      setTypedSearch(target.slice(0, i));
      if (i >= target.length) clearInterval(iv);
    }, 80);
    return () => clearInterval(iv);
  }, [stage]);
            ...
            <motion.div key="home" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-2 gap-2.5">
              ...
              >
                ...
                <div className="flex gap-1 mb-2">
                  {['bg-green-900/60','bg-purple-900/60','bg-blue-900/60'].map((c,i) => (
                    <div key={i} className={`w-8 ${c} rounded-sm overflow-hidden`}><div className="aspect-[3/4]" /></div>
                  ))}
                </div>
              ...
              <div className="bg-[#1a1f2e] border rounded-xl p-2.5" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
                <div className="flex items-center justify-between mb-2 gap-1">
                  <h3 className="text-[10px] font-semibold text-white truncate flex-1">Indie Gems</h3>
                ...
                <div className="h-10 rounded-md bg-white/5 border border-dashed border-white/10 flex items-center justify-center mb-2">
                  <span className="text-[8px] text-gray-500">No games yet</span>
                </div>
                <div className="flex items-center justify-between text-[8px] text-gray-500">
                  <span>0 games</span>
                  <span className="text-[#3498DB] flex items-center gap-0.5">Open <ChevronRight size={9} /></span>
              ...
              <div className="border-2 border-dashed border-white/10 rounded-xl p-2.5 flex flex-col items-center justify-center gap-1 text-gray-500 min-h-[80px] col-span-1">
                <Plus size={16} /><span className="text-[9px]">Add new list</span>
              </div>
            </motion.div>
          )}

          {/* ── Inside list view: search to add games ── */}
          {showListView && (
            <motion.div key="listview" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }} className="flex flex-col gap-2.5 h-full">
              {/* List header */}
              <div className="flex items-center gap-2">
                <Gamepad2 size={14} className="text-[#3498DB]" />
                <span className="text-xs font-semibold text-white">My Favourite RPGs</span>
                <span className="text-[8px] text-gray-500 ml-auto">0 games</span>
              </div>
              ...
              <div className="bg-[#1a1f2e] border border-white/10 rounded-xl p-2.5">
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <span className="text-[9px] text-white font-medium">Search Games</span>
                </div>
                <div className="w-full bg-black/30 border border-white/10 rounded-lg px-2 py-1.5 text-[9px] text-white flex items-center">
                  {typedSearch || <span className="text-gray-500">Search by game title…</span>}
                ...
                <AnimatePresence>
                  {typedSearch.length > 3 && (
                    <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="mt-2 space-y-1.5">
                      {['Elden Ring', 'Elden Ring Nightreign'].map((name, i) => (
                        <div key={i} className={`flex items-center gap-2 p-1.5 rounded-lg ${i === 0 && stage >= 7 ? 'bg-[#3498DB]/20 border border-[#3498DB]/30' : 'bg-white/5'}`}>
                          <div className="w-7 h-9 bg-amber-900/60 rounded-sm shrink-0" />
                          <div className="flex-1 min-w-0">
                            <p className="text-[9px] text-white font-medium truncate">{name}</p>
                            <p className="text-[7px] text-gray-500">FromSoftware</p>
                          </div>
                          <motion.div
                            animate={i === 0 && stage === 7 ? { scale: [1, 0.9, 1] } : { scale: 1 }}
                            className={`shrink-0 text-[7px] px-1.5 py-0.5 rounded font-medium ${i === 0 && stage >= 7 ? 'bg-green-500/20 text-green-400' : 'bg-[#3498DB]/20 text-[#3498DB]'}`}
                          >
                ...
                {gameAdded && (
                  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-[#1a1f2e] border border-white/10 rounded-xl p-2.5">
                    ...
                    <div className="flex gap-1.5">
                      <div className="w-10 bg-amber-900/60 rounded-sm"><div className="aspect-[3/4]" /></div>
                      <div className="flex-1">
                        <p className="text-[9px] text-white font-medium">Elden Ring</p>
                        <p className="text-[7px] text-gray-500">FromSoftware</p>
                      </div>
                    </div>
                  </motion.div>
              ...
              <div className="space-y-2">
                <div>
                  ...
                  <div className="bg-black/30 border border-white/10 rounded-lg px-2.5 py-1.5 text-[9px] text-white min-h-[26px]">
                    {typedListName || <span className="text-gray-500">e.g. My Favorite RPGs</span>}
