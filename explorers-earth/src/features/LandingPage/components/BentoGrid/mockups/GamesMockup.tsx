...
// Phase 0 (3s)  — Hero section + horizontal list cards overview (static)
// Phase 1 (3.5s) — Drill into "Top Picks" list → 2-col item grid, Recommendations tab active
...
const TOP_PICK = {
  ...
  summary: 'A vast open-world action RPG from FromSoftware and George R.R. Martin. Conquer a shattered world.',
...
const LISTS = [
  {
    ...
    status: 'Published',
    items: [
      { name: 'Elden Ring', meta: 'FromSoftware', img: '/landing/Paris.jpg' },
      { name: 'Cyberpunk 2077', meta: 'CDPR', img: '/landing/Bali.jpg' },
      { name: 'GTA V', meta: 'Rockstar', img: '/landing/Kyoto.jpg' },
      { name: 'Tekken 7', meta: 'Namco', img: '/landing/Eiffel_Tower.jpg' },
    ],
  },
  {
    name: 'Indie Gems',
  ...
  },
  {
    ...
    status: 'Public',
    items: [
      { name: 'Hollow Knight', meta: 'Team Cherry', img: '/landing/Louvre_Museum.jpg' },
      { name: 'Hades', meta: 'Supergiant', img: '/landing/Bali.jpg' },
    ],
  },
];
                ...
                <div className="flex items-end gap-2.5">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1 text-[7px] text-white/70 font-bold uppercase tracking-widest mb-2">
                      ...
                      <span>{TOP_PICK.genres.slice(0, 2).join(' / ')}</span>
                    </div>
                    <button className="flex items-center gap-1 bg-[#3498DB] text-white text-[7px] font-black py-1 px-2 rounded-md shadow-lg">
                      <Gamepad2 size={7} /> See Details
                    </button>
                  </div>
                  <div className="shrink-0 w-12 aspect-[3/4] rounded-md border border-white/20 overflow-hidden shadow-2xl">
                  ...
                  {list.items.length > 0 ? (
                    <div className="flex gap-1 mb-2">
                      {list.items.slice(0, 4).map((it, i) => (
                        <div key={i} className="w-8 rounded-sm overflow-hidden aspect-[3/4] bg-white/10 shrink-0">
                    ...
                    <div className="h-8 rounded-md bg-white/5 border border-dashed border-white/10 flex items-center justify-center mb-2">
                      <span className="text-[7px] text-gray-500">No games yet</span>
                    </div>
                  )}
                  <div className="flex items-center justify-between">
                    <span className="text-[8px] text-gray-500">{list.count} games</span>
                    <span className="text-[8px] text-[#3498DB] flex items-center gap-0.5 font-bold">Open <ChevronRight size={8} /></span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {(phase === 1 || phase === 2) && (
          <motion.div key="detail" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.3 }}
            ...
            <div className="px-3 py-2.5 flex items-center gap-2 shrink-0 border-b border-white/5">
              ...
              <Gamepad2 size={12} className="text-[#3498DB] shrink-0" />
              <div className="flex-1 min-w-0">
                <h2 className="text-[10px] font-black text-white truncate">{activeList.name}</h2>
                <p className="text-[7px] text-white/40 uppercase tracking-widest font-bold">{activeList.count} games • Games</p>
              </div>
            </div>
              ...
              {phase === 2 && (
                <motion.div key="manage" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  ...
                  <div>
                    ...
                    <p className="text-[7px] text-white/40 uppercase tracking-[0.2em] font-bold mt-0.5">{activeList.name} • {activeList.count} games</p>
                  </div>
