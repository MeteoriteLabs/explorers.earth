import { memo } from "react";

interface FeedIconProps {
  fill?: string; // SVG fill color, defaults to currentColor to inherit from text color
  size?: number | string; // Pixel value or tailwind arbitrary value; when number provided used for width/height
  className?: string; // Optional extra classes (e.g., to override size via Tailwind size-6)
}

// Feed (grid) icon composed of 4 rounded squares.
const FeedIcon = memo(
  ({
    fill = "currentColor",
    size = 24,
    className = "size-6",
  }: FeedIconProps) => {
    const dimensionProps =
      typeof size === "number"
        ? { width: size, height: size }
        : { width: 24, height: 24 };
    // If user passes a number we directly set width/height; if they pass a string (like '1.5rem') they can control via className instead.
    return (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
        fill={fill}
        {...dimensionProps}
        className={className}
      >
        <path
          fillRule="evenodd"
          clipRule="evenodd"
          d="M4.857 3A1.857 1.857 0 0 0 3 4.857v4.286C3 10.169 3.831 11 4.857 11h4.286A1.857 1.857 0 0 0 11 9.143V4.857A1.857 1.857 0 0 0 9.143 3zm10 0A1.857 1.857 0 0 0 13 4.857v4.286c0 1.026.831 1.857 1.857 1.857h4.286A1.857 1.857 0 0 0 21 9.143V4.857A1.857 1.857 0 0 0 19.143 3zm-10 10A1.857 1.857 0 0 0 3 14.857v4.286C3 20.169 3.831 21 4.857 21h4.286A1.857 1.857 0 0 0 11 19.143v-4.286A1.857 1.857 0 0 0 9.143 13zm10 0A1.857 1.857 0 0 0 13 14.857v4.286c0 1.026.831 1.857 1.857 1.857h4.286A1.857 1.857 0 0 0 21 19.143v-4.286A1.857 1.857 0 0 0 19.143 13z"
        />
      </svg>
    );
  }
);

export default FeedIcon;
