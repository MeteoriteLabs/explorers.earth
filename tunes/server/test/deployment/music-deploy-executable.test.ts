...
`;
  ...
  function run(operation: "bootstrap" | "deploy" | "rollback", requestedDigest: string, requestedCommit: string, options: RunOptions = {}) {
    ...
    Object.assign(environment, {
        ...
        MUSIC_DEPLOY_TEST_CURRENT_MARKER: options.expectedMarkerOverride ?? "0008_credential_revocation_operations",
  ...
  it.each([
  ...
  ])("upgrades authenticated historical marker %s directly to production 0008", (historicalMarker) => {
    ...
    expect(readFileSync(join(root, "deployment-state/music-schema-floor.tsv"), "utf8"))
      .toContain("\t0008_credential_revocation_operations\tpending\t");
    ...
    expect(readFileSync(join(root, "deployment-state/music-schema-floor.tsv"), "utf8"))
      .toContain("\t0008_credential_revocation_operations\tcurrent\t");
  }, 40_000);
  ...
  it("accepts a legitimate pending schema floor ahead of promoted ledger history", () => {
    ...
    for (const [relativePath, schema] of [
      ...
      const payload = [schema, repository, digest("b"), commit("b"), "0008_credential_revocation_operations", "pending"].join("\t");
      writeFileSync(join(root, relativePath), [schema, digest("b"), commit("b"), "0008_credential_revocation_operations", "pending",
        createHmac("sha256", hmacSentinel).update(payload).digest("hex")].join("\t") + "\n");
    }
    writeFileSync(eventLog, "");
    ...
    expect(readFileSync(join(root, "deployment-state/secure-images.tsv"), "utf8"))
      .toContain(`\t${digest("b")}\t${commit("b")}\t0008_credential_revocation_operations\t`);
  }, 20_000);
  ...
  it("keeps the HMAC key out of helper argv and environment while producing stable MACs", () => {
    ...
    const expectedPayload = ["music-ledger-v2", repository, "1", digest("a"), commit("a"), "0008_credential_revocation_operations", "GENESIS"].join("\t");
