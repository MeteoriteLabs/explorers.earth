...
describePostgres("C3 PostgreSQL 15 migration chain", () => {
  ...
  it("migrates a fresh database, creates all 27 runtime tables, verifies, and repeats as a no-op", async () => {
    ...
    expect(first.appliedIds).toEqual(["0001_runtime_baseline", "0002_identity_lifecycle", "0003_identity_lifecycle_hardening", "0004_identity_delete_saga", "0005_resource_bound_deletion_history", "0006_numeric_identity_lock", "0007_identity_provider_snapshot", "0008_credential_revocation_operations"]);
  ...
  it("serializes concurrent migrators and rolls a deliberately failing migration back atomically", async () => {
    ...
    expect([...left.appliedIds, ...right.appliedIds].sort()).toEqual(["0001_runtime_baseline", "0002_identity_lifecycle", "0003_identity_lifecycle_hardening", "0004_identity_delete_saga", "0005_resource_bound_deletion_history", "0006_numeric_identity_lock", "0007_identity_provider_snapshot", "0008_credential_revocation_operations"]);
    const failure = createMigrationDefinition("0009_deliberate_failure", "CREATE TABLE must_rollback(id integer); SELECT missing_function();");
    await expect(migrateMusicDatabase(pool, {
      migrations: [...loadMusicMigrations(), failure],
      testOnlyExpectedIds: ["0001_runtime_baseline", "0002_identity_lifecycle", "0003_identity_lifecycle_hardening", "0004_identity_delete_saga", "0005_resource_bound_deletion_history", "0006_numeric_identity_lock", "0007_identity_provider_snapshot", "0008_credential_revocation_operations", "0009_deliberate_failure"],
    })).rejects.toThrow();
    expect((await pool.query("SELECT to_regclass('public.must_rollback') AS value")).rows[0].value).toBeNull();
    expect((await pool.query("SELECT count(*)::int AS count FROM music_schema_migrations WHERE id='0009_deliberate_failure'")).rows[0].count).toBe(0);
    await secondPool.end();
    await pool.end();
  });

  it("rejects an appended production chain before any fresh or migrated database write", async () => {
    const appended = createMigrationDefinition("0009_unapproved", "CREATE TABLE forbidden_chain_write(id integer);\n");
  ...
  it("fails closed on checksum changes, catalog drift, missing/future migrations, and unversioned application tables", async () => {
    ...
    await expect(migrateMusicDatabase(pool, { migrations: [
      createMigrationDefinition(chain[0].id, `${chain[0].sql}\n-- tampered`), chain[1], chain[2], chain[3], chain[4], chain[5], chain[6], chain[7],
