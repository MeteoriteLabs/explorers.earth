...
const paths = {
  ...
  "/api/playlist/currently-playing": {
    post: ownerOperation({ summary: "Set or complete the owner playing song", status: "200", response: { oneOf: [ref("Song"), ref("PlaybackCommandResponse")] }, request: body(ref("PlayingInput"), "Song identifier or null to complete playback; new clients include the expected owner queue revision and the server advances it on success"), errors: {
      ...
      "409": failure("A newer owner playback intent is already canonical.", ["PLAYBACK_REVISION_CONFLICT"]),
    } }),
  },
...
export const MUSIC_OPENAPI_DOCUMENT = {
  ...
  components: {
    ...
    schemas: {
      ...
      PlayingInput: { type: "object", additionalProperties: false, required: ["songId"], properties: { songId: { type: ["integer", "null"], minimum: 1 }, expectedRevision: { type: "integer", minimum: 0 } } },
      PlaybackCommandResponse: { type: "object", additionalProperties: false, required: ["version", "revision", "song"], properties: { version: { type: "string", const: "music-playback/v1" }, revision: { type: "integer", minimum: 1 }, song: { oneOf: [ref("Song"), { type: "null" }] } } },
