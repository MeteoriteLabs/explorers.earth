...
export const MUSIC_OPENAPI_DOCUMENT = {
  ...
  components: {
    ...
    schemas: {
      ...
      Song: { allOf: [ref("SongInput"), { type: "object", required: ["id", "position", "status"], properties: { id: { type: "integer", minimum: 1 }, position: { type: "integer", minimum: 0 }, status: { type: "string", enum: ["queued", "playing", "played"] }, playedAt: { type: ["string", "null"], format: "date-time" } } }] },
      PlaylistSong: { allOf: [ref("SongInput"), { type: "object", required: ["id", "playlistId", "position"], properties: { id: { type: "integer", minimum: 1 }, playlistId: { type: "integer", minimum: 1 }, position: { type: "integer", minimum: 0 } } }] },
      Playlist: { type: "object", required: ["id", "name"], properties: { id: { type: "integer", minimum: 1 }, name: { type: "string" }, description: { type: ["string", "null"] }, isVisibleToGuests: { type: "boolean" }, songs: { type: "array", items: ref("PlaylistSong") } } },
      Dashboard: { type: "object", required: ["songs", "playedSongs"], properties: { songs: { type: "array", items: ref("Song") }, currentlyPlaying: { oneOf: [ref("Song"), { type: "null" }] }, playedSongs: { type: "array", items: ref("Song") } } },
      PublicPlaylist: { allOf: [ref("Dashboard"), { type: "object", properties: { user: { type: "object" }, playlists: { type: "array", items: ref("Playlist") } } }] },
      ...
      EntitlementResponse: { type: "object", additionalProperties: false, required: ["state", "paidMutation", "coreRead", "coreMutation", "maxAgeSeconds"], properties: { state: { type: "string", enum: ["unknown", "entitled", "denied", "revoked"] }, sourceUpdatedAt: { type: "string", format: "date-time" }, paidMutation: { type: "boolean" }, coreRead: { type: "boolean", const: true }, coreMutation: { type: "boolean", const: true }, maxAgeSeconds: { type: "integer", const: 600 } } },
      ...
      YouTubeVideo: { type: "object", additionalProperties: true },
      YouTubeSearchResponse: { type: "object", required: ["items"], properties: { items: { type: "array", items: ref("YouTubeVideo") }, nextPageToken: { type: ["string", "null"] } } },
    },
  },
} as const;
