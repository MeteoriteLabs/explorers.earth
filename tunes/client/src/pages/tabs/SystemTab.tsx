  ...
  const updateAppUrlMutation = useMutation({
    mutationFn: async (data: AppUrlSettingsFormData) => {
      const res = await fetch("/api/admin/system-settings/app_url", {
        ...
        body: JSON.stringify({ value: data.app_url }),
      });
    ...
    onSuccess: () => {
      toast({
        title: "URL updated",
        description:
          "Application URL has been updated successfully. Email verification links will now use this URL.",
      });
    ...
    <div className="space-y-4">
      <div>
        <p className="text-sm text-muted-foreground mb-4">
          Set the application's base URL that will be used for email verification links and other external
          references. This should include the protocol (https://) and domain without trailing slash.
        </p>
        <div className="p-3 rounded-md border-l-4 border-amber-500 bg-amber-50 dark:bg-amber-950/20 mb-4">
          <p className="text-sm text-amber-800 dark:text-amber-300">
            <strong>Important:</strong> This URL is critical for email verification. Users will receive
            verification links containing this URL. If deployed to a different domain, make sure to update this
            setting.
          </p>
        </div>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>Example: https://myapp.example.com</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
            <div className="flex items-center space-x-4">
              <Button type="submit" disabled={updateAppUrlMutation.isPending || !form.formState.isDirty}>
                {updateAppUrlMutation.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                Save URL
              </Button>
      ...
      <div>
        ...
        <div className="p-4 rounded-md border bg-muted/40">
          <p className="text-sm mb-1">
            <span className="font-semibold">Current URL:</span> {appUrlSetting?.value || "Not set"}
          </p>
          <p className="text-sm mb-1">
            <span className="font-semibold">Environment:</span> {process.env.NODE_ENV || "development"}
          </p>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">Average query execution time</p>
              </CardContent>
            </Card>
          </div>
        </CardContent>
            ...
            <div className="space-y-2">
              ...
              <div className="rounded-md border">
                {(systemData?.recentErrors || []).map((error: any, index: number) => (
                  <div
                    key={index}
                    className={cn("flex items-center justify-between p-4", index !== 0 && "border-t")}
                  >
                    <div>
                ...
                {(!systemData?.recentErrors || systemData.recentErrors.length === 0) && (
                  <div className="p-4 text-center text-sm text-muted-foreground">No recent errors</div>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between space-x-2">
              <div>
                ...
                <p className="text-sm text-muted-foreground">Maximum requests per minute per IP</p>
              </div>
              <Input type="number" className="w-24" value={systemData?.rateLimit || 60} onChange={() => {}} />
            </div>
            <Separator />
            <div className="flex items-center justify-between space-x-2">
              <div>
              ...
              <Switch checked={systemData?.debugMode || false} onCheckedChange={() => {}} />
            </div>
            <Separator />
            <div className="flex items-center justify-between space-x-2">
              <div>
                <Label>Maintenance Mode</Label>
                <p className="text-sm text-muted-foreground">Temporarily disable non-admin access</p>
              </div>
              <Switch checked={systemData?.maintenanceMode || false} onCheckedChange={() => {}} />
            </div>
          </div>
        </CardContent>
      </Card>
