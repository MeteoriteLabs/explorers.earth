...
import { MUSIC_COMPOSE_PROJECT } from "./music-compose-safety.ts";
...
const fixtureCleanupIntentName = "music-fixture-cleanup.intent";
const fixtureCleanupIntentTemporaryName = /^\.music-fixture-cleanup-intent-[a-f0-9]{32}\.tmp$/;
const fixtureCleanupIntentSchema = "music-fixture-cleanup/v1";
...
export class FixtureUnsupportedLegacyEnvironmentError extends Error {
  readonly code = "MUSIC_FIXTURE_LEGACY_ENVIRONMENT_UNSUPPORTED";
  constructor() {
    super(`Raw fixture environment authority is unsupported; use the explicit guarded cleanup/re-bootstrap command: npm run music:bootstrap -- --mode fixture --confirm-project ${MUSIC_COMPOSE_PROJECT}`);
    this.name = "FixtureUnsupportedLegacyEnvironmentError";
  }
}

export class FixtureCleanupScopeError extends Error {
  readonly code = "MUSIC_FIXTURE_CLEANUP_SCOPE_REFUSED";
  constructor() {
    super(`Fixture cleanup requires the exact project confirmation ${MUSIC_COMPOSE_PROJECT}`);
    this.name = "FixtureCleanupScopeError";
  }
}

export class FixtureCleanupPendingError extends Error {
  readonly code = "MUSIC_FIXTURE_CLEANUP_PENDING";
  constructor() {
    super(`Fixture cleanup is pending; rerun the explicit guarded cleanup/re-bootstrap command for ${MUSIC_COMPOSE_PROJECT}`);
    this.name = "FixtureCleanupPendingError";
  }
}

export type FixtureUnsupportedCleanupKind = "token" | "environment" | "journal" | "temporary" | "pointer" | "intent";
export type FixtureUnsupportedCleanupPhase = "before-open" | "before-truncate" | "after-truncate" | "after-fsync" | "after-close";

export interface FixtureUnsupportedCleanupDependencies {
  afterAuthorityCaptured?: () => void;
  beforeLeafOpen?: (kind: FixtureUnsupportedCleanupKind, path: string) => void;
  onCleanupPhase?: (kind: FixtureUnsupportedCleanupKind, phase: FixtureUnsupportedCleanupPhase) => void;
  platform?: NodeJS.Platform;
  descriptor?: {
    truncate?: (descriptor: number, kind: FixtureUnsupportedCleanupKind) => void;
    sync?: (descriptor: number, kind: FixtureUnsupportedCleanupKind) => void;
    close?: (descriptor: number, kind: FixtureUnsupportedCleanupKind) => void;
  };
  runWindowsEraseHelper?: (
    leaf: FixtureCleanupLeafAuthority,
    directories: FixtureCleanupDirectoryAuthority[],
...
function runWindowsBoundEraseHelper(
  leaf: FixtureCleanupLeafAuthority,
  directories: FixtureCleanupDirectoryAuthority[],
): { status: number | null; error?: Error } {
  if (!leaf.windows || directories.some((directory) => !directory.windows)) {
    return { status: null, error: new Error("native cleanup authority unavailable") };
  }
  return runWindowsNativeHelper([
    "erase",
    leaf.path,
    ...encodeWindowsNativeIdentity(leaf.windows, true),
    String(directories.length),
    ...directories.flatMap((directory) => [
      directory.path,
      ...encodeWindowsNativeIdentity(directory.windows!, false),
    ]),
  ]);
}
...
function inspectWindowsNativeIdentity(path: string): FixtureWindowsNativeIdentity {
  ...
  try {
    ...
    if (!/^[a-f0-9]{8}$/.test(parsed.volumeSerial)
        ...
        || !(parsed.sha256 === "-" || (/^[a-f0-9]{64}$/.test(parsed.sha256) && parsed.size <= 131_072))) {
...
export interface FixtureCleanupDirectoryAuthority {
  path: string;
  stat: BigIntStats;
  windows?: FixtureWindowsNativeIdentity;
}

export interface FixtureCleanupLeafAuthority {
  kind: FixtureUnsupportedCleanupKind;
  path: string;
  stat: BigIntStats;
  size: number;
  sha256: string;
...
interface FixtureCleanupScopeInventory {
  directory: string;
  names: string[];
  pattern: RegExp;
  existed: boolean;
...
interface FixtureUnsupportedCleanupAuthority {
  root: string;
  directories: FixtureCleanupDirectoryAuthority[];
  leaves: FixtureCleanupLeafAuthority[];
  inventories: FixtureCleanupScopeInventory[];
}

interface FixtureCleanupIntentRecord {
  schema: typeof fixtureCleanupIntentSchema;
  project: typeof MUSIC_COMPOSE_PROJECT;
  rootDev: string;
  rootIno: string;
  rootWindows: Pick<FixtureWindowsNativeIdentity, "volumeSerial" | "fileId" | "attributes" | "linkCount"> | null;
}

export function cleanupUnsupportedFixtureEnvironmentForRebootstrap(
  repositoryRoot: string,
  confirmedProject: string,
  dependencies: FixtureUnsupportedCleanupDependencies = {},
): void {
  const root = resolve(repositoryRoot);
  if (confirmedProject !== MUSIC_COMPOSE_PROJECT) throw new FixtureCleanupScopeError();
  let authority: FixtureUnsupportedCleanupAuthority;
  try {
    authority = captureUnsupportedCleanupAuthority(root, dependencies.platform ?? process.platform);
  } catch (error) {
    if (error instanceof FixtureUnsupportedLegacyEnvironmentError) throw error;
    if (error instanceof FixtureSecretCleanupError) throw error;
    if (error instanceof FixtureEnvironmentPersistenceError) throw new FixtureSecretCleanupError(error.targetId);
    throw new FixtureSecretCleanupError("fixture-cleanup-authority");
  }
  const intent = ensureFixtureCleanupIntent(authority, dependencies.platform ?? process.platform);
  dependencies.afterAuthorityCaptured?.();

  try {
    for (const leaf of authority.leaves.filter((candidate) => candidate.kind !== "pointer")) {
      eraseUnsupportedCleanupLeaf(authority, leaf, dependencies);
    }
    assertUnsupportedCleanupInventoryRetired(authority);
    const pointer = authority.leaves.find((candidate) => candidate.kind === "pointer");
    if (!pointer) throw new FixtureSecretCleanupError("fixture-environment-authority");
    eraseUnsupportedCleanupLeaf(authority, pointer, dependencies);
    assertUnsupportedCleanupInventoryRetired(authority);
    eraseUnsupportedCleanupLeaf(authority, intent, dependencies);
  } catch (error) {
    if (error instanceof FixtureSecretCleanupError) throw error;
    throw new FixtureSecretCleanupError("fixture-cleanup-authority");
  }
...
function captureUnsupportedCleanupAuthority(root: string, platform: NodeJS.Platform): FixtureUnsupportedCleanupAuthority {
  assertNoLinkedAncestors(root);
  assertOwnedDirectory(root);
  const state = inspectFixtureEnvironmentAuthority(root);
  if (state !== "unsupported" && state !== "tombstone") throw new FixtureUnsupportedLegacyEnvironmentError();
  const rootBeforeArtifacts = lstatSync(root, { bigint: true });
  const artifactDirectory = join(root, ".artifacts");
  if (!existsSync(artifactDirectory)) mkdirSync(artifactDirectory, { mode: 0o700 });
  assertOwnedDirectory(artifactDirectory);
  if (!sameIdentity(rootBeforeArtifacts, lstatSync(root, { bigint: true }))) throw fixtureSecretError();

  const directories = new Map<string, FixtureCleanupDirectoryAuthority>();
  const captureDirectory = (path: string) => {
    const absolute = resolve(path);
    const key = process.platform === "win32" ? absolute.toLowerCase() : absolute;
    if (directories.has(key)) return;
    const stat = lstatSync(absolute, { bigint: true });
    const underAuthorizedRoot = absolute === root
      || absolute.startsWith(`${root}${process.platform === "win32" ? "\\" : "/"}`);
    if (!stat.isDirectory() || stat.isSymbolicLink() || (underAuthorizedRoot && !ownedByCurrentUser(stat.uid))) {
      throw fixtureSecretError();
    }
    directories.set(key, {
      path: absolute,
      stat,
      windows: platform === "win32" ? inspectWindowsNativeIdentity(absolute) : undefined,
    });
  };
  const captureDirectoryChain = (path: string) => {
    for (const directory of exactDirectoryChain(path)) captureDirectory(directory);
  };
  captureDirectoryChain(root);

  const leaves: FixtureCleanupLeafAuthority[] = [];
  const captureLeaf = (path: string, kind: FixtureUnsupportedCleanupKind) => {
    const absolute = resolve(path);
    captureDirectoryChain(dirname(absolute));
    assertNoLinkedAncestors(absolute);
    const opened = openAndReadOwnedFileAllowEmpty(absolute, 131_072, true);
    try {
      leaves.push({
        kind,
        path: absolute,
        ...
        windows: platform === "win32" && opened.bytes.length > 0 ? inspectWindowsNativeIdentity(absolute) : undefined,
      });
    } finally {
      closeDescriptor(opened.descriptor, {}, basename(absolute));
    }
  };

  const scopes: Array<{ directory: string; pattern: RegExp; kind: FixtureUnsupportedCleanupKind }> = [
    { directory: join(root, FIXTURE_MUSIC_TOKEN_SECRET_DIRECTORY_RELATIVE_PATH), pattern: fixtureTokenName, kind: "token" },
    { directory: join(root, FIXTURE_MUSIC_ENVIRONMENT_DIRECTORY_RELATIVE_PATH), pattern: fixtureEnvironmentGenerationName, kind: "environment" },
    {
      directory: join(root, ".artifacts", "music-rotation-journals"),
      pattern: /^(?:rotation-[a-f0-9]{32}\.json|\.rotation-(?:update|intent)-[a-f0-9]{32}\.tmp)$/,
      kind: "journal",
    },
    {
      directory: root,
      pattern: /^(?:\.env\.music\.test\.[a-f0-9]{32}\.tmp|\.env\.music\.test\.reference-[a-f0-9]{32}\.tmp)$/,
      kind: "temporary",
    },
    {
      directory: artifactDirectory,
      pattern: fixtureCleanupIntentTemporaryName,
      kind: "journal",
    },
  ];
  const inventories: FixtureCleanupScopeInventory[] = [];
  for (const scope of scopes) {
    const existed = existsSync(scope.directory);
    const names = existed ? readdirSync(scope.directory).filter((name) => scope.pattern.test(name)).sort() : [];
    inventories.push({ directory: scope.directory, names, pattern: scope.pattern, existed });
    if (existed) captureDirectoryChain(scope.directory);
    for (const name of names) captureLeaf(join(scope.directory, name), scope.kind);
  }

  const pointer = join(root, ".env.music.test");
  if (!existsSync(pointer)) throw new FixtureUnsupportedLegacyEnvironmentError();
  captureLeaf(pointer, "pointer");
  const order: Record<FixtureUnsupportedCleanupKind, number> = {
    token: 0,
    environment: 1,
    journal: 2,
    temporary: 3,
  ...
  leaves.sort((left, right) => order[left.kind] - order[right.kind] || left.path.localeCompare(right.path));
  return { root, directories: Array.from(directories.values()), leaves, inventories };
}

function fixtureCleanupIntentPath(root: string): string {
  return join(root, ".artifacts", fixtureCleanupIntentName);
}

function assertNoPendingFixtureCleanup(root: string): void {
  const path = fixtureCleanupIntentPath(root);
  if (!existsSync(path)) return;
  try {
    assertNoLinkedAncestors(path);
    const stat = lstatSync(path, { bigint: true });
    assertOwnedRegularFile(stat);
    if (stat.size > BigInt(0)) throw new FixtureCleanupPendingError();
  } catch (error) {
    if (error instanceof FixtureCleanupPendingError) throw error;
    throw new FixtureCleanupPendingError();
  }
}

function cleanupIntentRecord(authority: FixtureUnsupportedCleanupAuthority): FixtureCleanupIntentRecord {
  const root = authority.directories.find((directory) => sameResolvedPath(directory.path, authority.root));
  if (!root) throw new FixtureSecretCleanupError("fixture-cleanup-root");
  return {
    schema: fixtureCleanupIntentSchema,
    project: MUSIC_COMPOSE_PROJECT,
    rootDev: root.stat.dev.toString(),
    rootIno: root.stat.ino.toString(),
    rootWindows: root.windows ? {
      volumeSerial: root.windows.volumeSerial,
      fileId: root.windows.fileId,
      attributes: root.windows.attributes,
      linkCount: root.windows.linkCount,
    } : null,
  };
}

function parseCleanupIntent(contents: Buffer): FixtureCleanupIntentRecord {
  let parsed: unknown;
  try { parsed = JSON.parse(contents.toString("utf8")); }
  catch { throw new FixtureSecretCleanupError("fixture-cleanup-intent"); }
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new FixtureSecretCleanupError("fixture-cleanup-intent");
  }
  const value = parsed as Record<string, unknown>;
  if (Object.keys(value).sort().join(",") !== "project,rootDev,rootIno,rootWindows,schema"
      || value.schema !== fixtureCleanupIntentSchema
      || value.project !== MUSIC_COMPOSE_PROJECT
      || typeof value.rootDev !== "string" || !/^[0-9]+$/.test(value.rootDev)
      || typeof value.rootIno !== "string" || !/^[0-9]+$/.test(value.rootIno)) {
    throw new FixtureSecretCleanupError("fixture-cleanup-intent");
  }
  if (value.rootWindows !== null) {
    if (!value.rootWindows || typeof value.rootWindows !== "object" || Array.isArray(value.rootWindows)) {
      throw new FixtureSecretCleanupError("fixture-cleanup-intent");
    }
    const windows = value.rootWindows as Record<string, unknown>;
    if (Object.keys(windows).sort().join(",") !== "attributes,fileId,linkCount,volumeSerial"
        || typeof windows.volumeSerial !== "string" || !/^[a-f0-9]{8}$/.test(windows.volumeSerial)
        || typeof windows.fileId !== "string" || !/^[a-f0-9]{16}$/.test(windows.fileId)
        || !Number.isSafeInteger(windows.attributes) || !Number.isSafeInteger(windows.linkCount)) {
      throw new FixtureSecretCleanupError("fixture-cleanup-intent");
    }
  }
  return value as unknown as FixtureCleanupIntentRecord;
}

function captureCleanupLeafAuthority(
  path: string,
  kind: FixtureUnsupportedCleanupKind,
  platform: NodeJS.Platform,
): FixtureCleanupLeafAuthority {
  const opened = openAndReadOwnedFileAllowEmpty(path, 131_072, true);
  try {
    return {
      kind,
      path: resolve(path),
      ...
      size: opened.bytes.length,
      sha256: createHash("sha256").update(opened.bytes).digest("hex"),
      windows: platform === "win32" && opened.bytes.length > 0 ? inspectWindowsNativeIdentity(path) : undefined,
    };
  } finally {
    closeDescriptor(opened.descriptor, {}, basename(path));
  }
...
function sameCleanupIntent(left: FixtureCleanupIntentRecord, right: FixtureCleanupIntentRecord): boolean {
  return JSON.stringify(left) === JSON.stringify(right);
}

function ensureFixtureCleanupIntent(
  authority: FixtureUnsupportedCleanupAuthority,
  platform: NodeJS.Platform,
): FixtureCleanupLeafAuthority {
  const path = fixtureCleanupIntentPath(authority.root);
  const expected = cleanupIntentRecord(authority);
  if (existsSync(path)) {
    const existing = captureCleanupLeafAuthority(path, "intent", platform);
    if (existing.size > 0) {
      const bytes = readFileSync(path);
      if (createHash("sha256").update(bytes).digest("hex") !== existing.sha256
          || !sameCleanupIntent(parseCleanupIntent(bytes), expected)) {
        throw new FixtureSecretCleanupError("fixture-cleanup-intent");
      }
      return existing;
    }
  ...
  const temporary = join(artifactDirectory, `.music-fixture-cleanup-intent-${secureRandomBytes(16).toString("hex")}.tmp`);
  const contents = Buffer.from(`${JSON.stringify(expected)}\n`, "utf8");
  const descriptor = openSync(temporary, constants.O_CREAT | constants.O_EXCL | constants.O_RDWR, 0o600);
  let failed = false;
  try {
    ...
    while (offset < contents.length) {
      ...
      if (written < 1) throw fixtureSecretError();
      offset += written;
    }
    fsyncSync(descriptor);
  } catch { failed = true; }
  try { closeSync(descriptor); } catch { failed = true; }
  if (failed) throw new FixtureSecretCleanupError("fixture-cleanup-intent");

  let destination: FixtureReplaceDestinationAuthority | undefined;
  if (existsSync(path)) {
    const stat = lstatSync(path, { bigint: true });
    assertOwnedRegularFile(stat);
    const bytes = readFileSync(path);
    if (bytes.length !== 0) throw new FixtureSecretCleanupError("fixture-cleanup-intent");
    destination = {
      stat,
      sha256: createHash("sha256").update(bytes).digest("hex"),
      windows: platform === "win32" ? inspectWindowsNativeIdentity(path) : undefined,
    };
  }
  replaceFixtureMetadataDurably(temporary, path, { platform }, destination);
  const intent = captureCleanupLeafAuthority(path, "intent", platform);
  if (!sameCleanupIntent(parseCleanupIntent(readFileSync(path)), expected)) {
    throw new FixtureSecretCleanupError("fixture-cleanup-intent");
  }
  return intent;
}

function exactDirectoryChain(path: string): string[] {
  const absolute = resolve(path);
...
function directoriesForCleanupLeaf(
  authority: FixtureUnsupportedCleanupAuthority,
  leaf: FixtureCleanupLeafAuthority,
): FixtureCleanupDirectoryAuthority[] {
  const byPath = new Map(authority.directories.map((directory) => [
    process.platform === "win32" ? directory.path.toLowerCase() : directory.path,
    directory,
  ]));
  return exactDirectoryChain(dirname(leaf.path)).map((path) => {
    const directory = byPath.get(process.platform === "win32" ? path.toLowerCase() : path);
    if (!directory) throw new FixtureSecretCleanupError("fixture-cleanup-directory");
    return directory;
  });
}

function assertCleanupDirectoryAuthorityCurrent(directories: FixtureCleanupDirectoryAuthority[]): void {
  for (const directory of directories) {
    const current = lstatSync(directory.path, { bigint: true });
    if (!current.isDirectory() || current.isSymbolicLink()
        || !sameIdentity(directory.stat, current)
        || directory.stat.mode !== current.mode
        || directory.stat.nlink !== current.nlink) {
      throw new FixtureSecretCleanupError("fixture-cleanup-directory");
    }
  }
}

function assertCleanupLeafCurrent(leaf: FixtureCleanupLeafAuthority, allowRetired = false): BigIntStats {
  const current = lstatSync(leaf.path, { bigint: true });
  assertOwnedRegularFile(current);
  if (!sameIdentity(leaf.stat, current)
      || leaf.stat.mode !== current.mode
      || leaf.stat.nlink !== current.nlink
      || (!allowRetired && current.size !== BigInt(leaf.size))
      || (allowRetired && current.size !== BigInt(0) && current.size !== BigInt(leaf.size))) {
    throw new FixtureSecretCleanupError(leaf.kind);
  }
  return current;
}

function eraseUnsupportedCleanupLeaf(
  authority: FixtureUnsupportedCleanupAuthority,
  leaf: FixtureCleanupLeafAuthority,
  dependencies: FixtureUnsupportedCleanupDependencies,
): void {
  dependencies.beforeLeafOpen?.(leaf.kind, leaf.path);
  dependencies.onCleanupPhase?.(leaf.kind, "before-open");
  const directories = directoriesForCleanupLeaf(authority, leaf);
  assertCleanupDirectoryAuthorityCurrent(directories);
  const current = assertCleanupLeafCurrent(leaf, true);
  if (current.size === BigInt(0)) {
    dependencies.onCleanupPhase?.(leaf.kind, "after-close");
    return;
  }
  dependencies.onCleanupPhase?.(leaf.kind, "before-truncate");
  const platform = dependencies.platform ?? process.platform;
  if (platform === "win32" && !dependencies.descriptor) {
    const result = dependencies.runWindowsEraseHelper
      ? dependencies.runWindowsEraseHelper(leaf, directories)
      : runWindowsBoundEraseHelper(leaf, directories);
    if (result.error || result.status !== 0) throw new FixtureSecretCleanupError(leaf.kind);
    dependencies.onCleanupPhase?.(leaf.kind, "after-truncate");
    dependencies.onCleanupPhase?.(leaf.kind, "after-fsync");
    const retired = assertCleanupLeafCurrent(leaf, true);
    if (retired.size !== BigInt(0)) throw new FixtureSecretCleanupError(leaf.kind);
    assertCleanupDirectoryAuthorityCurrent(directories);
    dependencies.onCleanupPhase?.(leaf.kind, "after-close");
    return;
  }
  ...
  try {
    for (const directory of directories) {
      const directoryDescriptor = openSync(directory.path, constants.O_RDONLY);
      directoryDescriptors.push(directoryDescriptor);
      const openedDirectory = fstatSync(directoryDescriptor, { bigint: true });
      if (!openedDirectory.isDirectory() || !sameIdentity(directory.stat, openedDirectory)) throw fixtureSecretError();
    }
    assertCleanupDirectoryAuthorityCurrent(directories);
  } catch {
    for (const directoryDescriptor of directoryDescriptors.reverse()) {
      try { closeSync(directoryDescriptor); } catch { /* the typed failure below remains authoritative */ }
    }
    throw new FixtureSecretCleanupError("fixture-cleanup-directory");
  }
  let descriptor: number;
  try { descriptor = openSync(leaf.path, constants.O_RDWR | noFollow, 0o600); }
  catch {
    for (const directoryDescriptor of directoryDescriptors.reverse()) {
      try { closeSync(directoryDescriptor); } catch { /* the typed failure below remains authoritative */ }
    }
  ...
  try {
    ...
    if (!sameIdentity(leaf.stat, opened)
        || bytes.length !== leaf.size
        || createHash("sha256").update(bytes).digest("hex") !== leaf.sha256
        || !sameIdentity(opened, lstatSync(leaf.path, { bigint: true }))) throw fixtureSecretError();
    assertCleanupDirectoryAuthorityCurrent(directories);
    (dependencies.descriptor?.truncate ?? ((file) => ftruncateSync(file, 0)))(descriptor, leaf.kind);
    dependencies.onCleanupPhase?.(leaf.kind, "after-truncate");
    (dependencies.descriptor?.sync ?? ((file) => fsyncSync(file)))(descriptor, leaf.kind);
    dependencies.onCleanupPhase?.(leaf.kind, "after-fsync");
  } catch { failed = true; }
  try { (dependencies.descriptor?.close ?? ((file) => closeSync(file)))(descriptor, leaf.kind); }
  catch { failed = true; }
  for (const directoryDescriptor of directoryDescriptors.reverse()) {
    try { closeSync(directoryDescriptor); } catch { failed = true; }
  }
  if (failed) throw new FixtureSecretCleanupError(leaf.kind);
  dependencies.onCleanupPhase?.(leaf.kind, "after-close");
}

function assertUnsupportedCleanupInventoryRetired(authority: FixtureUnsupportedCleanupAuthority): void {
  assertCleanupDirectoryAuthorityCurrent(authority.directories);
  for (const inventory of authority.inventories) {
    if (existsSync(inventory.directory) !== inventory.existed) throw new FixtureSecretCleanupError("fixture-cleanup-inventory");
    if (!inventory.existed) continue;
    const currentNames = readdirSync(inventory.directory).filter((name) => inventory.pattern.test(name)).sort();
    if (currentNames.length !== inventory.names.length
        || currentNames.some((name, index) => name !== inventory.names[index])) {
      throw new FixtureSecretCleanupError("fixture-cleanup-inventory");
    }
  }
  for (const leaf of authority.leaves.filter((candidate) => candidate.kind !== "pointer")) {
    if (assertCleanupLeafCurrent(leaf, true).size !== BigInt(0)) {
      throw new FixtureSecretCleanupError(leaf.kind);
    }
  }
}
...
export function rotateFixtureMusicAuthority(
  ...
  const root = resolve(repositoryRoot);
  assertNoPendingFixtureCleanup(root);
  assertSupportedFixtureEnvironmentAuthority(root);
  recoverFixtureAuthorityRotations(root);
...
type FixtureEnvironmentAuthorityState = "missing" | "tombstone" | "reference" | "unsupported";

export function inspectFixtureEnvironmentAuthority(repositoryRoot: string): FixtureEnvironmentAuthorityState {
  const root = resolve(repositoryRoot);
  const pointerPath = join(root, ".env.music.test");
  if (!existsSync(pointerPath)) return "missing";
  let opened: ReturnType<typeof openAndReadOwnedFileAllowEmpty> | undefined;
  try {
    opened = openAndReadOwnedFileAllowEmpty(pointerPath, 65_536, false);
    if (opened.bytes.length === 0) return "tombstone";
    parseFixtureEnvironmentReference(opened.bytes.toString("ascii"));
    return "reference";
  } catch {
    return "unsupported";
  } finally {
    if (opened) closeDescriptor(opened.descriptor, {}, basename(pointerPath));
  }
}

function assertSupportedFixtureEnvironmentAuthority(root: string): void {
  if (inspectFixtureEnvironmentAuthority(root) === "unsupported") {
    throw new FixtureUnsupportedLegacyEnvironmentError();
  }
}

function readFixtureEnvironmentForRotation(root: string): { contents?: string } | undefined {
  const state = inspectFixtureEnvironmentAuthority(root);
  ...
  try {
    ...
    const pointer = readCurrentPointerBytes(root);
    const parsed = parseFixtureEnvironmentReference(pointer.toString("ascii"));
...
function readCurrentPointerBytes(root: string): Buffer {
  ...
  const opened = openAndReadOwnedFileAllowEmpty(path, 65_536, false);
...
function readPreviousFixtureEnvironmentAuthority(root: string, referencePath: string): {
  ...
  const opened = openAndReadOwnedFileAllowEmpty(referencePath, 65_536, false);
  if (opened.bytes.length === 0) {
  ...
  let parsed: FixtureEnvironmentReference;
  try {
    parsed = parseFixtureEnvironmentReference(contents);
  } catch {
    closeDescriptor(opened.descriptor, {}, basename(referencePath));
    throw new FixtureUnsupportedLegacyEnvironmentError();
  }
